...
class Binarization:
    ...
    def otsu(image, with_gaussian = False):
        ...
        if with_gaussian == True:
            ...
            _, output = cv2.threshold(blur,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
            return output
...
class NoiseRemoval:
    ...
    def crimmins_speckle_removal(image):
        ...
        total_iterations = 2 * (image.shape[0] - 2) * (image.shape[1] - 2)  # Total number of iterations
        ...
        for _ in range(2):
            for i in range(1, image.shape[0] - 1):
                for j in range(1, image.shape[1] - 1):
                    ...
                    neighbors = [output[i-1, j], output[i+1, j], output[i, j-1], output[i, j+1]]
