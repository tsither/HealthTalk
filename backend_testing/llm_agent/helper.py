...
def generate_query(llm, template, question, db):
    ...
    prompt = template.format(schema=get_schema(db),question=question, current_date=current_time)       #format template to include all necessary information (schema, question)
